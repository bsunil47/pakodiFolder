...
<li><a class="<?php if ($this->router->fetch_class()!= 'content') { ?>collapsed<?php } ?>" data-toggle="collapse" href="#togglePages8"><i class="menu-icon icon-screenshot"></i>
            <i class="icon-chevron-down pull-right"></i><i class="icon-chevron-up pull-right"></i>Content List</a>
            <ul id="togglePages8" class="unstyled <?php if (($this->router->fetch_class() == 'content') || ($this->router->fetch_class()== 'content' && ($this->router->fetch_method() == 'update'))) { ?>in<?php } ?> collapse">
