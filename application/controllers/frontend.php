...
class Frontend extends My_Controller {
	...
	public function forgotpassword() {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->common_model->initialise('users');
                $user_record = $this->common_model->get_record_single(array('email' => $_POST['email'],'status'=>1), '*');
                if (!empty($user_record)) {
                    ...
                    $this->setFlashmessage('error', 'Not registered with us or User in Inactive Mode');
                }
   ...
   public function logout() {
	...
	private function s3upload($filepath, $type)
    {
        ...
        $fp1 = $fil1[$c1 - 1];
        $ff = explode('.', $fp1);
        return $this->s3->putObject($file1, 'sprintmediasg', "$type/$fp1");
    }
