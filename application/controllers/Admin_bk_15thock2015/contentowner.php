...
class Contentowner extends My_Controller {
    ...
    public function add() {
        ...
        if (isset($_POST['submit'])) {
            if ($this->form_validation->run('addmoderator') == FALSE) {
                ...
                $result = $this->communication_model->send_password(array('contentowner_id' => $contentowner_id, 'name' => $this->input->post('name'), 'email' => $this->input->post('email'), 'password' => $this->input->post('password')));
		        if($result){
                $this->setFlashmessage('success', 'Contentowner added successfully');
                redirect(base_url() . "Admin/contentowner");
                }else{
                $this->setFlashmessage('error', 'Please Try again');    
                redirect(base_url() . "Admin/contentowner/add");
            }
            }
        }
        $this->layout->view($this->view_dir, $data);
    ...
    public function update($id) {
        ...
        if (!empty($_POST)) {
            ...
            $this->setFlashmessage('error', 'Please Try again'); 
            redirect(base_url() . "Admin/contentowner/update");
        }
        }
        $this->layout->view($this->view_dir, $data);
    }
