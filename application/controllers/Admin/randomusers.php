...
//        $data['rusers'] = $this->common_model->get_records(0, 'C.id,C.name,C.language_id,C.email,C.mobile,C.status,L.language', 0,$order);
    ...
    public function add()
    {
        ...
        if (!empty($_FILES) && is_uploaded_file($_FILES['excel_file']['tmp_name']) && ($FileType =='xlx' || $FileType == 'csv' || $FileType == 'xlsx')) {
            $basic_array = array();
            ...
            $i = $i + 1;
			 ...
			 $status = $aRow['status'];
                if ($status == 1) {
					...
					$link='<a href="' . base_url() . 'Admin/randomusers/randomstatus/'. $aRow['id'] . '/' . $aRow['status'] . '"><i class="iconred icon-remove" title="Inactive"></i></a>';
               } else if ($status == 0 || $status == '' || $status == "NULL") {
                    $statusn = '<i class="iconred icon-remove" title="Inactive"></i>';
					$link=$link='<a href="' . base_url() . 'Admin/randomusers/randomstatus/'. $aRow['id'] . '/' . $aRow['status'] . '"><i class="icongreen icon-ok" title="Active"></i></a>';
                }
