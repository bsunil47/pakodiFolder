     ...
     public function update($id,$lang) {
      ...
      //  print_r($data['category']);
        if (!empty($_POST)) {
           ...
           echo json_encode(['code' => 200,'url' => base_url() . "Admin/category"]); exit;
            //redirect(base_url() . "Admin/category");
            }else{
            $this->setFlashmessage('error', 'Please Try again');  
            //echo json_encode(['code' => 200,'url' => base_url() . "Admin/category/update"]);
   ...
   public function getData() {
    ...
    $row[2] = $aRow['datecreated'];
           /* $status = $aRow['status'];
            ...
            $row[3] = '&nbsp;&nbsp;<span class="open-div" data-url="' . base_url() . 'Admin/category/update/' . $aRow['cat_id'] .'/' . $aRow['language_id'] .'"><i class="iconred icon-pencil" title="Edit"></i></span>';
            $output['aaData'][] = $row;
        }
