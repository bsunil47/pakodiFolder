...
//        $data['flags'] = $this->common_model->get_records(0, '*', $where);
	...
	$this->common_model->initialise('master_content');
	$this->common_model->array = array('flag1_count' => 0,'flag2_count' => 0,'flag3_count' => 0,'flag4_count' => 0,'flag5_count' => 0,'flag6_count' => 0);
	$where = array('master_content_id' => $id);
	$this->common_model->update($where);
	$this->session->set_flashdata('getmsg','Reset Successfully');
	redirect(base_url() . "Admin/flags");
	}
         ...
         foreach ($data['result'] as $aRow) {
            ...
            foreach ($aColumns as $col) {
              $row[] = $aRow[$col];
				...
				$row[1] = '<span class="open-div" data-url="' . base_url() . 'Admin/content/view/' . $aRow['master_content_id'] . '">'.$aRow["master_content_id"].'</span>';
              ...
              $row[] = '<span class="open-div" data-url="' . base_url().'Admin/flags/othersview/' . $aRow['master_content_id'] .'"><i class="iconred icon-eye-open" title="View"></i></a>&nbsp;&nbsp;'
                        . '<a href="' . base_url().'Admin/flags/changestatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><i class="iconred icon-off" title="Disable"></i></a>&nbsp;&nbsp;'
						. '<a href="' . base_url().'Admin/flags/reset/' . $aRow['master_content_id'] . '"><i class="iconred icon-refresh" title="Reset"></i></a>';
