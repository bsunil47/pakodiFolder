...
class Content extends My_Controller {
	...
	public function delcontentstatus($id, $status) {
        if ($status == 0) {
            $statusn = 2;
        }
        if ($status == 2) {
       ...
       if($update==0){
        ...
        redirect(base_url() . "Admin/content/deletedcontentlist");
    }
    ...
    public function getData() { //echo "I am in getdata method";
        ...
        $language_id ="";
        if(!empty($_POST['sSearch_3'])){
            $language_id = " AND MC.language_id=" .$_POST['sSearch_3'];
        }
        $category_id = "";
        if (!empty($_POST['sSearch_2'])) {
            $category_id = " AND MC.category_id =" . $_POST['sSearch_2'];
        }
        ...
        $aColumns = array('MC.master_content_id','MC.unique_code', 'C.title','MC.thumb_filename','MC.clip_length','MC.contentclip_filename', 'MC.content_activationdate', 'MC.content_expirydate', 'MC.content_status', 'MC.recommend_type','MC.content_type','MC.contentshare_count','MC.contentdownload_count','MC.contentplay_count');
		$this->common_model->initialise('master_content as MC');
        $this->common_model->join_tables="content as C";
        $this->common_model->join_on="MC.master_content_id = C.master_content_id";
		...
		$daterange = "";
        if(!empty($_POST['sSearch_0']) && !empty($_POST['sSearch_1'])){
		 //$daterange = " AND DATE_FORMAT(MC.datecreated,'%m/%d/%Y') between '".$_POST['sSearch_0']."' and '".$_POST['sSearch_1']."'";
		 $daterange = " AND DATE_FORMAT(MC.datecreated,'%m/%d/%Y') between '".$_POST['sSearch_0']."' and '".$_POST['sSearch_1']."'";
        }
		$where = "C.language_id = 1 AND MC.content_status = 1".$daterange.$category_id.$language_id;

        $data = $this->common_model->getTable($aColumns, $where,'MC.datecreated','desc','master_content_id');
        $output = $data['output'];
        ...
        foreach ($data['result'] as $aRow) {
			...
			$row[0] = '<span class="open-div" data-url="' . base_url() . 'Admin/content/view/' . $aRow['master_content_id'] . '">'.$aRow["unique_code"].'</span>';
			$row[1] = $aRow['title'];
            $i = $i + 1;
			
			//$row[2] = '<img src="http://sprintmedia.s3.amazonaws.com/appimages/'.$aRow["thumb_filename"].'"  class="test" width="120" height="80">';
			$row[2] = '<div class="tric test"><img src="http://sprintmediasg.s3.amazonaws.com/appimages/'.$aRow["thumb_filename"].'"  width="80" height="auto"><div class="redo"><b>'.$aRow['clip_length'].' S'.'</b></div></div>';
			
            if($aRow['content_type']==2){
                $typenew = "<audio src='http://sprintmediasg.s3.amazonaws.com/audios/".$aRow['contentclip_filename']."'  width='50' height='auto' controls ></audio>";
                
            }else if($aRow['content_type']==1){
                $typenew = "<video src='http://sprintmediasg.s3.amazonaws.com/videos/".$aRow['contentclip_filename']."' class='test' width='50' height='auto' controls ></video>";
            }
            //$row[3] =$typenew;
            ...
            if ($aRow['content_activationdate'] != '0000-00-00' && $aRow['content_activationdate'] != NULL) {
                ...
                $row[3] = $actdate->format('M jS Y');
            }
            $row[4] = '-';
            if ($aRow['content_expirydate'] != '0000-00-00' && $aRow['content_expirydate'] != NULL) {
                $expdate = new DateTime($aRow['content_expirydate']);
                $row[4] = $expdate->format('M jS Y');
            }
            $status = $aRow['content_status'];
            if ($status == 1) {
                $statusn = '<i class="icongreen icon-ok" title="Active"></i>';
				$link='<a href="' . base_url() . 'Admin/content/contentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><i class="iconred icon-remove" title="Inactive"></i></a>&nbsp;';
            } else if ($status == 0 || $status == '' || $status == "NULL") {
                $statusn = '<i class="iconred icon-remove" title="Inactive"></i>';
				$link='<a href="' . base_url() . 'Admin/content/contentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><i class="icongreen icon-ok" title="Active"></i></a>&nbsp;';
            }
            $statusr = $aRow['recommend_type'];
            if ($statusr == 2) {
                $statusrn = '<i class="icongreen icon-ok" title="Rec Active"></i>';
				$rlink = '<a href="' . base_url() . 'Admin/content/recommendstatus/' . $aRow['master_content_id'] . '/' . $aRow['recommend_type'] . '"><i class="icongreen icon-ok" title="Active"></i></a>';
            } else if ($statusr == 1) {
                $statusrn = '<i class="iconred icon-remove" title="Rec Inactive"></i>';
				$rlink = '<a href="' . base_url() . 'Admin/content/recommendstatus/' . $aRow['master_content_id'] . '/' . $aRow['recommend_type'] . '"><i class="iconred icon-remove" title="Inactive"></i></a>';
            }
			$row[5] = $rlink;
            $row[6] = $statusn;
            $this->common_model->initialise('userdubs');
            $dubcount = $this->common_model->get_record_single(array('content_id' => $aRow['master_content_id']), 'count(*) as cnt');
            $row[7] = '<div>'.$link.'<span style="float:right" data-url="' . base_url() .
                'Admin/content/update/' . $aRow['master_content_id'] . '" title="Edit"
class="open-div"><i class="iconred icon-pencil" ></i></span></div>'.'<div style="text-align:center"><span class="play-content" title="Play" data-contenttype="'.$aRow['content_type'].'" data-media="'.$aRow['contentclip_filename'].'"><i class="iconred fa fa-play-circle-o fa-2"></i></span></div>'
                . '<div></div><span class="open-div" data-url="' .
                base_url() . 'Admin/content/view/' . $aRow['master_content_id'] . '"><i class="iconred icon-eye-open" title="View"></i></span>'
                .'<span style="float:right" class="report-content" title="Reports"
data-title="' .$aRow['title'].'" data-uniquecode="'.
                $aRow['unique_code'].'" data-media="'. $aRow['thumb_filename'].'"
data-sharecount="' . $aRow['contentshare_count'] . '"
data-downloadcount="' . $aRow['contentdownload_count'] . '"
data-viewcount="' . $aRow['contentplay_count'] . '" data-dubscount="' .
                $dubcount->cnt . '"><i class="iconred icon-table"
 ></i></span></div>';

            //$row[7] = $link.'<span data-url="' . base_url() . 'Admin/content/update/' . $aRow['master_content_id'] . '" title="Edit" class="open-div"><i class="iconred icon-pencil" ></i></span>'
             //       . '&nbsp;&nbsp;<span class="open-div" data-url="' . base_url() . 'Admin/content/view/' . $aRow['master_content_id'] . '"><i class="iconred icon-eye-open" title="View"></i></span>'
             //       .'<span class="report-content" title="Reports"  data-sharecount="' . $aRow['contentshare_count'] . '" data-downloadcount="' . $aRow['contentdownload_count'] . '" data-viewcount="' . $aRow['contentplay_count'] . '" data-dubscount="' . $dubcount->cnt . '"><i class="iconred icon-table" ></i></span>'.'&nbsp;<span class="play-content" title="Play" data-contenttype="'.$aRow['content_type'].'" data-media="'.$aRow['contentclip_filename'].'"><i class="iconred fa fa-play-circle-o"></i></span>';
					$output['aaData'][] = $row;
        }
    ...
    public function getRData() {
        ...
        foreach ($data['result'] as $aRow) {
            ...
            $row[2] = "<img src='$path' width='80' height='auto'>";
            if ($aRow['content_type'] == 2) {
                $apath = "http://sprintmediasg.s3.amazonaws.com/audios/" . $aRow['contentclip_filename'];
				$row[3]  = "<audio width='0' height='30' controls ><source src='$apath' type='audio/mpeg'></audio>";
            } else {
                $apath = "http://sprintmediasg.s3.amazonaws.com/videos/" . $aRow['contentclip_filename'];
	...
	$language_id ="";
        ...
        //$aColumns = array('MC.master_content_id', 'C.title', 'MC.content_activationdate', 'MC.content_expirydate', 'MC.content_status', 'MC.recommend_type');
		$aColumns = array('MC.master_content_id','MC.unique_code', 'C.title','MC.thumb_filename','MC.clip_length','MC.contentclip_filename', 'MC.content_activationdate', 'MC.content_expirydate', 'MC.content_status', 'MC.recommend_type','MC.content_type');
        $this->common_model->initialise('master_content as MC');
        $this->common_model->join_tables="content as C";
        $this->common_model->join_on="MC.master_content_id = C.master_content_id";
		...
		$where = "C.language_id = 1 AND MC.content_status = 2".$daterange.$category_id.$language_id;
        ...
        foreach ($data['result'] as $aRow) {
			...
			$row[2] = '<div class="tric test"><img src="http://sprintmediasg.s3.amazonaws.com/appimages/'.$aRow["thumb_filename"].'"  width="80" height="auto"><div class="redo"><b>'.$aRow['clip_length'].' S'.'</b></div></div>';
            if($aRow['content_type']==2){
                $typenew = "<audio src='http://sprintmediasg.s3.amazonaws.com/audios/".$aRow['contentclip_filename']."'  controls ></audio>";
            ...
            if ($status == 2) {
				...
				$statusn = '<i class="iconred icon-remove" title="Inactive"></i>';
				$link = '<a href="' . base_url() . 'Admin/content/delcontentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><i class="icongreen icon-ok" title="Active"></i></a>&nbsp;';
				...
				$statusn = '<i class="icongreen icon-ok" title="Active"></i>';
				$link = '<a href="' . base_url() . 'Admin/content/delcontentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><i class="iconred icon-remove" title="Inactive"></i></a>&nbsp;';
            }
            $row[5] = $statusn;

            $row[6] = $link.'<span data-url="' . base_url() . 'Admin/content/update/' . $aRow['master_content_id'] .'/deletedcontentlist'. '" title="Edit" class="open-div"><i class="iconred icon-pencil" ></i></span>'
			. '&nbsp;&nbsp;<span data-url="' . base_url() . 'Admin/content/view/' . $aRow['master_content_id'] . '" class="open-div" title="View"><i class="iconred icon-eye-open" ></i></span>'.'&nbsp;<span class="iconred  fa fa-play-circle-o play-content" title="Play" data-contenttype="'.$aRow['content_type'].'" data-media="'.$aRow['contentclip_filename'].'"></span>';
            $output['aaData'][] = $row;
        }
        echo json_encode($output);
    }
   ...
   /* Content Waiting for Approval Start */
    ...
    public function getWData()
    {
       $aColumns = array('U.id','U.name','count(MC.master_content_id)as count');
        ...
        foreach ($data['result'] as $aRow) {
            ...
            $row[1]=$aRow['name'];
            $row[2]=$aRow['count'];
            $row[3] = '<a href="' . base_url() . 'Admin/content/getwdetails/' . $aRow['id'].'"><i class="iconred icon-list" title="Details"></i></a>';
          $output['aaData'][] = $row;
        }
        echo json_encode($output); 
    }
    ...
    public function getWDData2(){//echo $this->input->post('id');exit;
		$category_id = "";
        if (!empty($_POST['sSearch_0'])) {
            $category_id = " AND MC.category_id =" . $_POST['sSearch_0'];
        }
        $aColumns = array('MC.master_content_id', 'C.title', 'MC.content_activationdate', 'MC.content_expirydate');
        $this->common_model->initialise('master_content as MC');
        $this->common_model->join_tables="content as C";
        $this->common_model->join_on="MC.master_content_id = C.master_content_id";
		$where = "MC.contentowner_id = '".$_POST['id']."' AND MC.content_status = 0".$category_id;
	...
	$language_id = "";
        if(!empty($_POST['sSearch_1'])){
            $language_id = " AND MC.language_id =". $_POST['sSearch_1'];
        }
        $category_id = "";
        if (!empty($_POST['sSearch_0'])) {
            $category_id = " AND MC.category_id =" . $_POST['sSearch_0'];
        }
        $aColumns = array('MC.master_content_id','MC.unique_code', 'C.title','MC.thumb_filename','MC.clip_length','MC.contentclip_filename', 'MC.content_activationdate', 'MC.content_expirydate', 'MC.content_type');
        $this->common_model->initialise('master_content as MC');
        $this->common_model->join_tables="content as C";
        $this->common_model->join_on="MC.master_content_id = C.master_content_id";
		$where = "MC.contentowner_id = '".$_POST['id']."' AND MC.content_status = 0".$category_id.$language_id;
        //$where = array("MC.contentowner_id"=>$_POST['id'],"MC.content_status"=>0);
//        if(!empty($_POST['sSearch_0']) && !empty($_POST['sSearch_1'])){
//		 $where = "DATE_FORMAT(MC.datecreated,'%m/%d/%Y') between '".$_POST['sSearch_0']."' and '".$_POST['sSearch_1']."'";
//        }
        ...
        foreach ($data['result'] as $aRow) {
            ...
            if($aRow['content_type']==2){
                ...
                $typenew = "<video src='http://sprintmediasg.s3.amazonaws.com/videos/".$aRow['contentclip_filename']."' class='test' width='50' height='auto' controls ></video></a>";
            }
            $row[3] ='<i class="iconred fa fa-play-circle-o fa-2 play-content" title="Play" style="padding:25px;" data-contenttype="'.$aRow['content_type'].'" data-media="'.$aRow['contentclip_filename'].'">';
            $row[4] = '-';
            if ($aRow['content_activationdate'] != '0000-00-00' && $aRow['content_activationdate'] != NULL) {
                $actdate = new DateTime($aRow['content_activationdate']);
                $row[4] = $actdate->format('M jS Y');
            }
            $row[5] = '-';
            if ($aRow['content_expirydate'] != '0000-00-00' && $aRow['content_expirydate'] != NULL) {
                $expdate = new DateTime($aRow['content_expirydate']);
                $row[5] = $expdate->format('M jS Y');
            }
            $row[6] = '<input type="checkbox" class="checkbox_s" onclick="chk(this)" name="approve[]" id="ch'.$j.'" value="'. $aRow['master_content_id'] .'">';
            $output['aaData'][] = $row;
        }
        echo json_encode($output);
    }
