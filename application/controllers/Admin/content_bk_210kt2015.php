...
class Content extends My_Controller {
    ...
    public function getData() { //echo "I am in getdata method";
        ...
        $aColumns = array('MC.master_content_id','MC.unique_code', 'C.title','MC.thumb_filename','MC.contentclip_filename', 'MC.content_activationdate', 'MC.content_expirydate', 'MC.content_status', 'MC.recommend_type','MC.content_type');
		$this->common_model->initialise('master_content as MC');
        $this->common_model->join_tables="content as C";
        $this->common_model->join_on="MC.master_content_id = C.master_content_id";
		...
		$where = "C.language_id = 1 AND MC.content_status = 1".$daterange.$category_id;
        ...
        foreach ($data['result'] as $aRow) {
            ...
            if($aRow['content_type']==2){
                $typenew = "<audio src='http://sprintmedia.s3.amazonaws.com/audios/".$aRow['contentclip_filename']."'  width='50' height='50' controls ></audio>";
                
            }else if($aRow['content_type']==1){
                $typenew = "<video src='http://sprintmedia.s3.amazonaws.com/videos/".$aRow['contentclip_filename']."' class='test' width='100' height='100' controls ></video>";
            }
            $row[3] =$typenew;
            ...
            if ($status == 1) {
				...
				$link='<a href="' . base_url() . 'Admin/content/contentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><button class="btn pakodi" title="status" style="border:1px solid #cccccc;">InActive</button></a>&nbsp;';
				...
				$link='<a href="' . base_url() . 'Admin/content/contentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><button class="btn pakodi" title="status" style="border:1px solid #cccccc;">Active</button></a>&nbsp;';
            }
            ...
            if ($statusr == 2) {
				...
				$rlink = '<a href="' . base_url() . 'Admin/content/recommendstatus/' . $aRow['master_content_id'] . '/' . $aRow['recommend_type'] . '"><img src="' . base_url() . '/appimages/tick.png"/></a>';
            } else if ($statusr == 1) {
                $statusrn = "<button class='btn-danger' title='Inactive' style='border:0px solid #cccccc;'>Rec Inactive</button>";
				$rlink = '<a href="' . base_url() . 'Admin/content/recommendstatus/' . $aRow['master_content_id'] . '/' . $aRow['recommend_type'] . '"><img src="' . base_url() . '/appimages/cros.png"/></a>';
            }
			$row[6] = $rlink;
            $row[7] = $statusn;

            $row[8] = $link.'<a href="' . base_url() . 'Admin/content/update/' . $aRow['master_content_id'] . '"><button class="btn pakodi" title="edit" style="border:1px solid #cccccc;">Edit</button></a>'
                    . '&nbsp;&nbsp;<a href="' . base_url() . 'Admin/content/view/' . $aRow['master_content_id'] . '"><button class="btn pakodi" title="edit" style="border:1px solid #cccccc;">View</button></a>&nbsp;';
            $output['aaData'][] = $row;
        }
    ...
    public function getMData() { //echo "I am in getdata method";
		...
		$where = "C.language_id = 1 AND MC.content_status = 2".$daterange.$category_id;
        ...
        foreach ($data['result'] as $aRow) {
            ...
            if($aRow['content_type']==2){
                ...
                $typenew = "<video src='http://sprintmedia.s3.amazonaws.com/videos/".$aRow['contentclip_filename']."' class='test' width='150' height='100' controls ></video>";
            }
            $row[3] =$typenew;
            $row[4] = '-';
            if ($aRow['content_activationdate'] != '0000-00-00' && $aRow['content_activationdate'] != NULL) {
            ...
            if ($status == 2) {
				...
				$link = '<a href="' . base_url() . 'Admin/content/delcontentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><button class="btn pakodi" title="status" style="border:1px solid #cccccc;">Active</button></a>&nbsp;';
				...
				$link = '<a href="' . base_url() . 'Admin/content/delcontentstatus/' . $aRow['master_content_id'] . '/' . $aRow['content_status'] . '"><button class="btn pakodi" title="status" style="border:1px solid #cccccc;">InActive</button></a>&nbsp;';
            }
            $row[6] = $statusn;

            $row[7] = $link.'<a href="' . base_url() . 'Admin/content/update/' . $aRow['master_content_id'] .'/deletedcontentlist'. '"><button class="btn pakodi" title="edit" style="border:1px solid #cccccc;">Edit</button></a>'
			. '&nbsp;&nbsp;<a href="' . base_url() . 'Admin/content/view/' . $aRow['master_content_id'] . '"><button class="btn pakodi" title="edit" style="border:1px solid #cccccc;">View</button></a>';
            $output['aaData'][] = $row;
        }
        echo json_encode($output);
   ...
   /* Content Waiting for Approval Start */
    ...
    public function getWData()
    {
       $aColumns = array('U.id','U.name','count(MC.master_content_id)as count');
        ...
        foreach ($data['result'] as $aRow) {
            ...
            $row[1]=$aRow['name'];
            $row[2]=$aRow['count'];
            $row[3] = '<a href="' . base_url() . 'Admin/content/getwdetails/' . $aRow['id'].'"><button class="btn pakodi" title="status" style="border:1px solid #cccccc;">Details</button></a>';
          $output['aaData'][] = $row;
        }
        echo json_encode($output); 
    }
	...
	$category_id = "";
        ...
        $aColumns = array('MC.master_content_id','MC.unique_code', 'C.title','MC.thumb_filename','MC.contentclip_filename', 'MC.content_activationdate', 'MC.content_expirydate', 'MC.content_type');
        $this->common_model->initialise('master_content as MC');
        $this->common_model->join_tables="content as C";
        $this->common_model->join_on="MC.master_content_id = C.master_content_id";
		$where = "MC.contentowner_id = '".$_POST['id']."' AND MC.content_status = 0".$category_id;
...
//        }
        ...
        foreach ($data['result'] as $aRow) {
            ...
            if($aRow['content_type']==2){
                ...
                $typenew = "<video src='http://sprintmedia.s3.amazonaws.com/videos/".$aRow['contentclip_filename']."' class='test' width='150' height='100' controls ></video></a>";
            }
            $row[3] =$typenew;
            $row[4] = '-';
            if ($aRow['content_activationdate'] != '0000-00-00' && $aRow['content_activationdate'] != NULL) {
