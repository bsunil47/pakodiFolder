        ...
        if($_POST['rangetype'] == 'yearly'){
            ...
            for ($i = 0; $i < 12; $i++) {
                ...
                if(!empty($dsarraynew1)){
                    $data['activecount'][] = (integer)$dsarraynew1[0]->activation_count;}else{
                    $data['activecount'][] = 0;
                }
                //Registered Users
        ...
        elseif ($_POST['rangetype'] == 'weekly') {
            if(!empty($_POST['userweek'])){
                ...
                foreach( $period as $key => $date) {
                    ...
                    if(!empty($dsarraynew1)){
                        $data['activecount'][] = (integer)$dsarraynew1[0]->num;}else{
                        $data['activecount'][] = 0;
                    }
                    //Registered Users
                    $selectn = 'SUM(status) as register_count';
...
//Active and Register Users statistics(Weekly)
        ...
        foreach ($data['result'] as $aRow) {
                ...
                if ($status == 1) {
					$statusn = '<i class="icongreen icon-ok" title="Active"></i>';
                   	$link='<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] . '/userlist"><i class="iconred icon-remove" title="Status"></i></a>&nbsp;';
                    
                } else if ($status == 0 || $status == '' || $status == "NULL") {
					$statusn = '<i class="iconred icon-remove" title="Inactive"></i>';
					$link='<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] . '/userlist"><i class="icongreen icon-ok" title="Status"></i></a>&nbsp;';
    ...
    public function getMODData() {
        ...
        foreach ($data['result'] as $aRow) {
                ...
                if ($status == 1) {
					$statusn = '<i class="icongreen icon-ok" title="Active"></i>';
					$link = '<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] .'/moderatoruserlist "><i class="iconred icon-remove" title="Status"></i></a>&nbsp;';
                } else if ($status == 0 || $status == '' || $status == "NULL") {
                    $statusn = '<i class="iconred icon-remove" title="Inactive">';
					$link = '<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] .'/moderatoruserlist "><i class="icongreen icon-ok" title="Status"></i></a>&nbsp;';
				}
				$row[4] = $statusn;
    ...
    public function modupdate($id) {
        ...
        //update query
        if (!empty($_POST)) {
			if (preg_match('#\b^[a-z0-9]{32}\b$#', $_POST['password'] )){$passcheck=1;}else{$passcheck=2;}
            if ($this->form_validation->run('updatemoderator') == FALSE && $passcheck!=1) {
                $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
            } else {            
            $data = array('name' => $this->input->post('name'), 'email' => $this->input->post('email'), 'msisdn' => $this->input->post('msisdn'), 'password' => md5($this->input->post('password')), 'app_language' => $this->input->post('language_id'));
            $this->common_model->initialise('users');
    ...
    public function adminuserlist()
    {
     ...
     public function getAdData() {
        ...
        foreach ($data['result'] as $aRow) {
                ...
                if ($status == 1) {
				$statusn = '<i class="icongreen icon-ok" title="Active"></i>';
					$link = '<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] .'/adminuserlist "><i class="iconred icon-remove" title="Status"></i></a>&nbsp;';
                } else if ($status == 0 || $status == '' || $status == "NULL") {
                   $statusn = '<i class="iconred icon-remove" title="Inactive">';
				   $link = '<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] .'/adminuserlist "><i class="icongreen icon-ok" title="Status"></i></a>&nbsp;';
                }
     ...
     public function adupdate($id) {
        ...
        if (!empty($_POST)) {
				if (preg_match('#\b^[a-z0-9]{32}\b$#', $_POST['password'] )){$passcheck=1;}else{$passcheck=2;}
            if ($this->form_validation->run('updateadminuser') == FALSE && $passcheck!=1) {
                $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
            } else {            
            $data = array('name' => $this->input->post('name'), 'email' => $this->input->post('email'), 'msisdn' => $this->input->post('msisdn'),'password' => md5($this->input->post('password')));
            $this->common_model->initialise('users');
    ...
    public function contentownerlist()
    {
     ...
     public function getCOData() {
        ...
        foreach ($data['result'] as $aRow) {
                ...
                if ($status == 1) {
					$statusn = '<i class="icongreen icon-ok" title="Active"></i>';
                    $link = '<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] .'/contentownerlist"><i class="iconred icon-remove" title="Status"></i></a>&nbsp;';
                } else if ($status == 0 || $status == '' || $status == "NULL") {
                    $statusn = '<i class="iconred icon-remove" title="Inactive"></i>';
					$link = '<a href="' . base_url() . 'Admin/users/userstatus/' . $aRow['id'] . '/' . $aRow['status'] .'/contentownerlist"><i class="icongreen icon-ok" title="Status"></i></a>&nbsp;';
                }
