...
class Moderators extends My_Controller {
    ...
    public function index() { //App moderators
       ...
       /* $this->common_model->initialise('user_types');
        $result_id = $this->common_model->get_records(0, '*', array('user_type' => 3));
    ...
    public function add() {
        ...
        if (isset($_POST['submit'])) {
            if ($this->form_validation->run('addmoderator') == FALSE) {
                ...
                $data_usertype = array('user_id' => $this->db->insert_id(), 'user_type' => 3);
                $this->common_model->initialise('user_types');
                $this->common_model->array = $data_usertype;
                $this->common_model->insert_entry();
                $this->load->model('communication_model');
                $result = $this->communication_model->send_moderator(array('name' => $this->input->post('name'), 'email' => $this->input->post('email'), 'password' => $this->input->post('password')));
		        if($result){
                $this->setFlashmessage('success', 'Moderators added successfully');
                redirect(base_url() . "Admin/moderators");
                }else{
                $this->setFlashmessage('error', 'Please Try again');    
                redirect(base_url() . "Admin/moderators/add");
                }
            }
        }
        $this->layout->view($this->view_dir, $data);
    ...
    public function update($id) {
        ...
        if (!empty($_POST)) {
            ...
            $this->setFlashmessage('error', 'Please Try again');  
            redirect(base_url() . "Admin/moderators/update");
            }
        }
        }
        $this->layout->view($this->view_dir, $data);
    ...
    public function getData() {
        ...
        foreach ($data['result'] as $aRow) {
                ...
                $row[] = '<a href="' . base_url() . 'Admin/moderators/userstatus/' . $aRow['id'] . '/' . $aRow['status'] . '"><button class="btn" title="status" style="border:1px solid #cccccc;">Status</button></a>&nbsp;'
                . '<a href="' . base_url() . 'Admin/moderators/update/' . $aRow['id'] . '"><button class="btn" title="Edit" style="border:1px solid #cccccc;">Edit</button></a>';
                $output['aaData'][] = $row;
