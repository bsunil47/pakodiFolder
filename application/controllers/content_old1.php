                        ...
                        for ($row = 3; $row <= $highestRow; ++$row) {
                            ...
                            if (isset($rowdata[1]) && isset($rowdata[2]) && isset($rowdata[3]) && isset($rowdata[4]) && isset($rowdata[5]) && isset($rowdata[6]) && isset($rowdata[7]) && isset($rowdata[8]) && isset($rowdata[9]) && isset($rowdata[10]) && isset($rowdata[12]) && isset($rowdata[14]) && $rowdata[3] == $this->session->userdata['user_id']) {
                                if ($rowdata[1] == 'insert' || $rowdata[1] == 'Insert') {
                                    ...
                                    $this->server_upload($target_dir . $fileInfo['filename'] . "/JPEG/", $rowdata[12], 'appimages');
                                    if ($rowdata[9] == 2) {
                                        ...
                                        $this->s3upload($target_dir . $fileInfo['filename'] . "/MP3/", $audio_file_info['filename'] . '.mp3', 'audios');
                                         $this->server_upload($target_dir . $fileInfo['filename'] . "/MP3/", $audio_file_info['filename'] . '.mp3', 'audios');
                                        $this->s3upload($target_dir . $fileInfo['filename'] . "/AAC/", $audio_file_info['filename'] . '.aac', 'audios');
                                        $this->server_upload($target_dir . $fileInfo['filename'] . "/AAC/", $audio_file_info['filename'] . '.aac', 'audios');
                                        ...
                                        $this->server_upload($target_dir . $fileInfo['filename'] . "/MP4/", $rowdata[10], 'videos');
                                    }
                                    $cat_id = $this->catname($rowdata[6]);
                                    $lang_id = $this->language($rowdata[8]);
                                    $master_array = array('unique_code' => $rowdata[2], 'contentowner_id' => $this->session->userdata['user_id'], 'category_id' => $cat_id, 'parental_advisory' => $rowdata[7], 'content_type' => $rowdata[9], 'contentclip_filename' => $rowdata[10], 'search_keywords' => $rowdata[24], 'thumb_filename' => $rowdata[12], 'clip_length' => $rowdata[14], 'copyright' => $rowdata[20], 'content_expirydate' => $rowdata[21], 'content_status' => 1);
