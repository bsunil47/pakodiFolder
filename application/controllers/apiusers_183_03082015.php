...
class Apiusers extends My_Controller
{
    ...
    public function categorylist()
    {
        ...
        $where = array('status' => 1, 'language_id' => $_POST['language_id']);
                ...
                if ($_POST['type'] == 1) {
                    ...
                    if (!empty($_FILES)) {
                        ...
                        foreach ($_FILES as $key => $value) {
                            //$fil=json_encode($_FILES); echo $fil; exit;
                            ...
                            if (!in_array($imageFileType, $allwoed_extentions)) {
                                ...
                                if (move_uploaded_file($_FILES[$key]["tmp_name"], $target_file)) {
                                    $compress_pic = "ffmpeg -i $target_file -vf scale=320:240 $target_file";
                                    exec($compress_pic);
                                    sleep(15);
                                    $data_array = array($key => basename($_FILES[$key]["name"]));
                                    $this->s3upload($target_file, "userimages");
                                }
                            }
                        }
                    }
    ...
    public function contentlist()
    {
        ...
        if ($user && $user->userprefer_languages) {
            if (!empty($_POST['language_id'])) {
                $where .= " AND (language_id IN (" . $_POST['language_id'] . ") OR language_id IN (" . 1 . "))";
            } else {
                $where .= " AND language_id IN (" . 1 . ")";
            }
        ...
        if (!empty($_POST['filterby'])) {
            switch ($_POST['filterby']) {
                case 0:
                    $orderby = 'content_id';//latest
                    break;
                case 1:
                    $orderby = $trending_value;//trending
                    break;
                case 2:
                    $orderby = 'contentlike_count';//liked
                    break;
                case 3:
                    $orderby = 'content_rating';//rated
                    break;
                case 4:
                    $orderby = 'contentdownload_count';//downloaded
                    break;
                default:
                    $orderby = 'content_id';
            }
        ...
        $error_msg = false;
        $select = ['content_id', 'content_type', 'recommend_type', 'contentowner_id', 'unique_code', 'SUBSTRING(title, 1, 14) as title', 'short_desc', 'category_id', 'parental_advisory', 'language_id', 'contentclip_filename', 'search_keywords', 'thumb_filename', 'SUBSTRING(movie_name, 1, 14) as movie_name', 'SUBSTRING(main_artist, 1, 14) as main_artist', 'SUBSTRING(sub_artists, 1, 14) as sub_artists', 'film_director', 'music_director', 'dialog_writer', 'content_expirydate', 'content_activationdate', 'contentlike_count', 'contentshare_count', 'contentdownload_count', 'content_rating', 'contentplay_count', 'dub_count', 'clip_length', 'copyright', 'content_status', 'recommend_sort', 'datecreated'];
        //echo $select = "content_id, content_type, recommend_type, contentowner_id, unique_code, SUBSTRING(title, 1, 14) as title, short_desc, category_id, parental_advisory, language_id, contentclip_filename, search_keywords, thumb_filename, SUBSTRING(movie_name, 1, 14) as movie_name, SUBSTRING(main_artist, 1, 14) as main_artist, SUBSTRING(sub_artists, 1, 14) as sub_artists, film_director, music_director, dialog_writer, content_expirydate, content_activationdate, contentlike_count, contentshare_count, contentdownload_count, content_rating, contentplay_count, dub_count, clip_length, copyright, content_status, recommend_sort, datecreated"; exit;
        if (!empty($_POST['groupby'])) {
            ...
            $group_result = $this->common_model->get_records($limit, $select, $where, $orderby, 'desc', $groupby);
            if ($group_result) {
                foreach ($group_result as $rows) {
                    ...
                    if ($groupby == 'language_id') {
                        ...
                        $this->common_model->initialise('content');
                        //$results[$language->language] = $this->resultset($_POST['language_id'],$this->common_model->get_records(0, $select, $where1, $orderby, 'desc'));
                        $results[$language->language] = $this->common_model->get_records(0, $select, $where1, $orderby, 'desc');
                    } else {
                        $this->common_model->initialise('content');
                        //$results[$rows->$groupby] = $this->resultset($_POST['language_id'],$this->common_model->get_records(0, $select, $where1, $orderby, 'desc'));
                        $results[$rows->$groupby] = $this->common_model->get_records(0, $select, $where1, $orderby, 'desc', 'unique_code');
                    }
                }//foreach
            } else {
                $error_msg = true;
            ...
            $this->common_model->initialise('content');
            //$results['none'] = $this->resultset($_POST['language_id'],$this->common_model->get_records($limit, $select, $where, $orderby, 'desc', 'unique_code'));
            $results['none'] = $this->common_model->get_records($limit, $select, $where, $orderby, 'desc', 'unique_code');
    ...
    private function resultset($language_id, $result)
    {
    ...
    public function ratings()
    {
        ...
        if (!empty($_POST)) {
            ...
            if (empty($data['error'])) {
                ...
                $this->common_model->initialise('users');
                $userlikes = $this->common_model->get_record_single(array('id' => $_POST['user_id']), '*');
                if ($_POST['type'] == 1) {
                    ...
                    if (empty($getlikes)) {
                        ...
                        $usercount = $userlikes->userlike_count + 1;
                        $update = array('contentlike_count' => $count);
                        $userupdate = array('userlike_count' => $usercount);
                    } else {
                        $like = 1;
                        $count = $user_record->contentlike_count;
                        $usercount = $userlikes->userlike_count;
                ...
                if ($_POST['type'] == 2) {
                    ...
                    $usercount = $userlikes->usershare_count + 1;
                    $update = array('contentshare_count' => $count);
                    $userupdate = array('usershare_count' => $usercount);
                }
                if ($_POST['type'] == 3) {
                    ...
                    if (empty($get_downloads)) {
                        ...
                        $usercount = $userlikes->userdownload_count + 1;
                        $update = array('contentdownload_count' => $count);
                        $userupdate = array('userdownload_count' => $usercount);
                    } else {
                        $insert = true;
                        $count = $user_record->contentdownload_count + 1;
                        $usercount = $userlikes->userdownload_count + 1;
                ...
                // print_r($update_content);exit;
                if ($_POST['type'] != 4 && $_POST['type'] != 5) {
                    $this->common_model->initialise('users');
                    $this->common_model->array = $userupdate;
                ...
                if ($insert == TRUE && $update_content == FALSE) {
                    if ($_POST['type'] == 1) {
                        ...
                        $this->common_model->join_tables = array('usercontentlike as UL', 'users as U');
                        $this->common_model->join_on = array('C.content_id = UL.content_id', 'UL.user_id = U.id');
                        $d_array = array('C.content_id' => $_POST['cid'], 'U.id' => $_POST['user_id']);
                        $select = "C.*,UL.like_status";
                    } else if ($_POST['type'] == 4) {
                        $this->common_model->initialise('content as C');
                        $this->common_model->join_tables = array('usercontentrating as UR', 'users as U');
                        $this->common_model->join_on = array('C.content_id = UR.content_id', 'UR.user_id = U.id');
                        $d_array = array('C.content_id' => $_POST['cid'], 'U.id' => $_POST['user_id']);
                        $select = "C.*,UR.user_rating";
                    } else {
                        $this->common_model->initialise('content');
                        $d_array = array('content_id' => $_POST['cid']);
                        $select = "*";
                    }
                    $content = $this->common_model->get_record_single($d_array, $select);
    ...
    public function dubslist()
    {
        ...
        (!empty($_POST['start'])) ? $start = $_POST['start'] : $start = 0;
        $limit = array('limit' => $this->limitval, 'start' => $start);
        $this->common_model->initialise('userdubs as UD');
        $this->common_model->join_tables = "content as C";
        $this->common_model->join_on = "UD.dub_done_content_id = C.content_id";
        //   $where = array('isdub_moderate'=>1, 'dub_status'=>1, 'user_id'=> $_POST['user_id']);
        $where = array('UD.user_id' => $_POST['user_id'], 'UD.dub_status' => 1);
        $result = $this->common_model->get_records($limit, '*,UD.datecreated as dubcreateddate', $where);
        if (!empty($result)) {
    ...
    public function dubs()
    {
        ...
        if (!empty($_POST)) {
            if ($_POST['stype'] == 0) {
                $data_array1 = array('thumb_filename' => $_POST['thumb_name1'], 'dubclip_filename' => $_POST['video_name1']);
            }
                ...
                if (!in_array($imageFileType, $allwoed_extentions) && !in_array($imageFileType1, $allwoed_extentions1)) {
                    ...
                    if ($_POST['stype'] == 1) {
                        if (move_uploaded_file($_FILES['thumb_name']["tmp_name"], $target_file) && move_uploaded_file($_FILES['video_name']["tmp_name"], $target_file1)) {
                            $input = $target_file;
                            $output = $target_file;
                            $mix = "/usr/local/bin/ffmpeg -i  " . $input . " -vf transpose=1  " . $output;
                            exec($mix);
                            //  $this->s3upload($target_file1, "dubs");
            ...
            $this->common_model->initialise('content');
            $get_movie = $this->common_model->get_record_single(array('content_id' => $_POST['content_id']), '*');
            $dub_count = $get_movie->dub_count + 1;
            $data_array['movie_name'] = $get_movie->movie_name;
            $data_array['search_keywords'] = $get_movie->search_keywords;
            ...
            $data_array['clip_length'] = $data_array1['clip_length'] = $_POST['clip_length'];
            $data_array1['dub_done_content_id'] = $_POST['content_id'];
            $data_array1['dub_status'] = 1;
            if ($_POST['stype'] == 1) {
                $this->common_model->initialise('content');
                $this->common_model->array = $data_array;
            ...
            if ($get) {
                $this->common_model->initialise('content');
                $this->common_model->array = array('dub_count' => $dub_count);
                $this->common_model->update("content_id = '" . $_POST['content_id'] . "'");
                //$data['info'] = ['status' => 1, "Message" => "Sucess"];
                $data['info'] = ['status' => 1, "Message" => "Success", 'dub' => $get];
            } else {
    ...
    public function relatedcontentlist()
    {
        ...
        if (!empty($_POST['groupby'])) {
            ...
            switch ($_POST['groupby']) {
                case 1:
                    $where .= " AND (main_artist LIKE '%" . $_POST['name'] . "%' OR sub_artists LIKE '%" . $_POST['name'] . "%')";
                    $name = $_POST['name'];
                    break;
                case 2:
                    $where .= " AND movie_name LIKE '" . $_POST['name'] . "'";
                    $name = $_POST['name'];
                    break;
                default:
                    $name = 'none';
        ...
        $this->common_model->initialise('content');
        $result = $this->common_model->get_records($limit, '*', $where, $orderby, 'desc', 'unique_code');
    ...
    public function dubwatermark()
    {
        ...
        if (!empty($_POST)) {
            if (!empty($_FILES['videoname']['name'])) {
                $image = FCPATH . "zipfiles/application_50.png";
                $target_dir = FCPATH . "temp/";
                if (!is_dir($target_dir)) {
                    mkdir($target_dir, 0777, true);
                }
                $target_file1 = $target_dir . basename($_FILES["videoname"]["name"]);
                if (move_uploaded_file($_FILES['videoname']["tmp_name"], $target_file1)) {
                    $video_name = $target_file1;
                    $out_file = FCPATH . "temp/" . $_POST['user_id'] . date('his') . ".mp4";
                    $out_file1 = FCPATH . "dubs/" . $_POST['user_id'] . date('his') . ".mp4";
                    $out_file2 = "http://183.82.107.134:81/pakodi/dubs/" . $_POST['user_id'] . date('his') . ".mp4";
                    //  $out_file1 = "http://sprintmedia.s3.amazonaws.com/dubs/" . $_POST['user_id'] . date('his') . ".mp4";
                    $mix = " /usr/local/bin/ffmpeg -i " . $video_name . " -vf \"movie=" . $image . " [watermark]; [in][watermark] overlay=main_w-overlay_w-50:main_h-overlay_h-50 [out]\" " . $out_file;
                    exec($mix);
                    //print_r($out_file);
                    copy($out_file, $out_file1);
                    $data = ['status' => 1, "Message" => "Sucess", "watermarkfile" => $out_file2];
                }
            }
        }
        $output = $this->api_model->json($data, true);
    ...
    public function deletedownload()
    {
        ...
        $data = array();
        if (!empty($_POST)) {
            if ($_POST['content_id'] != "" && $_POST['user_id'] != "") {

                $this->common_model->initialise('usercontentdownload');
                $this->common_model->array = array('status' => 0);
                $delete = $this->common_model->delete_record();

                $where = array('user_id' => $_POST['user_id'], 'content_id' => $_POST['content_id']);
                $details = $this->common_model->get_records(0, 'ucd_id', $where);

                if (!empty($details)) {
                    $data['info'] = ["status" => 0, "Message" => "fail"];
                } else {
                    $data['info'] = ["status" => 1, "Message" => $_POST['content_id'] . " deleted Success"];
                }
            }
        } else {
            $data['info'] = ["status" => 0, "Message" => "Insufficient Data"];
