...
//        exit;
    ...
    private function mix_video($audio_file, $video_file, $video_file1, $pa, $target_dir) {
        ...
        // exec($mix);
        $mixi = "/usr/local/bin/ffmpeg -loop 1 -i $logo_file -i $audio_file -c:v libx264 -profile:v high -level 3.0 -c:a libfdk_aac -b:a 128k -shortest -vf scale=640:640 -pix_fmt yuv420p $video_file";
        //$mixi = "/usr/local/bin/ffmpeg -loop 1 -i  " . $logo_file . " -i  " . $audio_file . " -c:v libx264 -c:a libfdk_aac -b:a 128k -shortest  " . $video_file1;
        exec($mixi);
       ...
       // exec($mix);
        ...
        //$this->server_upload('', $video_file, 'sharevideo');
        unlink($video_file);
        //unlink($video_file1);
    }
                ...
                foreach ($file_csv as $key => $value) {
                    ...
                    if (is_file($target_dir . $fileInfo['filename'] . '/' . $value) && ($fileInfor['extension'] == 'xlsx' || $fileInfor['extension'] == 'xls')) {
                        ...
                        for ($row = 3; $row <= $highestRow; ++$row) {
                            ...
                            if (isset($rowdata[1]) && isset($rowdata[2]) && isset($rowdata[3]) && isset($rowdata[4]) && isset($rowdata[5]) && isset($rowdata[6]) && isset($rowdata[7]) && isset($rowdata[8]) && isset($rowdata[9]) && isset($rowdata[10]) && isset($rowdata[12]) && isset($rowdata[14]) && $rowdata[3] == $this->session->userdata['user_id'] && is_file($target_dir . $fileInfo['filename'] . "/media/". $rowdata[10]) && is_file($target_dir . $fileInfo['filename'] . "/media/". $rowdata[12])) {
                                //echo "testing";exit;
                                if ($rowdata[1] == 'insert' || $rowdata[1] == 'Insert' || $rowdata[1] == 'INSERT') {
                                    ...
                                    if (file_exists($target_file)) {
                                        ...
                                        $master_array = array('unique_code' => $rowdata[2],'language_id'=>$lang_id, 'contentowner_id' => $this->session->userdata['user_id'], 'category_id' => $cat_id, 'parental_advisory' => $rowdata[7], 'content_type' => $rowdata[9], 'contentclip_filename' => $rowdata[10], 'search_keywords' => $rowdata[11], 'thumb_filename' => $rowdata[12], 'clip_length' => $rowdata[14], 'copyright' => $rowdata[20], 'content_expirydate' => $rowdata[21], 'content_status' => 1,'vuser_id'=> $user_name->id, 'vuser_name'=> $user_name->name, 'vuser_type' => 1/*,'contentowner_name' => $user_name*/);
                        ...
                        if (isset($insert)) {
                            ...
                            //unlink($targett_dir . $fileInfo['basename']);
                            //return  $this->session->set_flashdata('result','The Job You Created Done Sucessfully');
                            //echo $this->setFlashmessage('result', 'The Job You Created Done Sucessfully');exit;
                            ...
                            return "Please Check the mandatory feilds in excel is set or not,click on download excel for sample";
                            //exit;
                            //echo $this->setFlashmessage('result', 'Please Check the mandatory feilds in excel is set or not,click on download excel for sample');exit;
    ...
    public function getData() { //echo "I am in getdata method";exit;
        ...
        //$id='2';
        $aColumns = array('MC.contentowner_id','C.title','MC.unique_code' ,'MC.content_type', 'MC.thumb_filename', 'MC.contentclip_filename', 'MC.content_status', 'MC.datecreated');
        ...
        $where = "MC.contentowner_id = '" . $this->session->userdata['user_id'] . "'".$language_id;
        ...
        foreach ($data['result'] as $aRow) {
            if ($aRow['contentowner_id'] == $this->session->userdata['user_id']) {
                ...
                $row[3] = "<img src='$path' width='50' height='50'>";
                ...
                if ($aRow['content_type'] == 2) {
                    ...
                    $row[4]  = "<audio src='$apath' width='0' height='30' controls ></audio>";
                    ...
                    $row[4]  = "<video src='$apath' width='0' height='150' controls ></video>";
                    //$row[4]='testing';
                }
                ...
                if ($status == 1) {
                    $statusn = "<span  title='Active' style='border:0px solid #cccccc;color:green'>Active</span>";
                } else if ($status == 0 || $status == '' || $status == "NULL") {
                    $statusn = "<span  title='Inactive' style='border:0px solid #cccccc;color:red'>Inactive</span>";
                }
                $row[5] = $statusn;
