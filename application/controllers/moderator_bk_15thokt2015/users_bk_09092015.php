...
class Users extends My_Controller {
    ...
    public function __construct() {
        ...
        if (!in_array($this->router->fetch_method(), $allowed_urls)) {
            if (!$this->_ismoderator_logged_in()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $result = $this->common_model->get_record_single($data, '*');
                 if (!empty($result)) {
                    ...
                    if (!empty($result_s)) {
                        $this->session->set_userdata('moderator_user_id', $result->id);
                        ...
                        $this->session->set_userdata('moderator_user_type', $result_s->user_type);
    ...
    public function changepassword() {
        ...
        if ($this->form_validation->run() == FALSE) { 
            ...
            if (!empty($_POST)) {
                $aid = $this->session->userdata('moderator_user_id');
                $this->common_model->initialise('users');
    ...
    public function logout() {
         $this->session->unset_userdata('moderator_user_id');
         ...
         $this->session->unset_userdata('moderator_user_type');
	...
	public function recordupdate($id) {
        ...
        if (!empty($_POST)) {
            ...
            unset($data['submit']);
            $this->common_model->initialise('userdubs');
            $this->common_model->array = array('isdub_moderate' => $_POST['isdub_moderate'], 'moderatedby' => $this->session->userdata('moderator_user_id'), 'moderatedon' => date('Y-m-d H:i:s'));
            $where = array('dub_id' => $id);
            $result_update = $this->common_model->update($where);
            $data['userdubs'] = $this->common_model->get_record_single(array('dub_id' => $id), '*');
            redirect(base_url() . "moderator/users/records");
	...
	public function moverecord($id) {
		...
		if (!empty($_POST)) { 
            ...
            if($this->common_model->insert_entry()){
                $this->common_model->initialise('userdubs');
                $this->common_model->array = array('isdub_moderate' => 1, 'moderatedby' => $this->session->userdata('moderator_user_id'), 'moderatedon' => date('Y-m-d H:i:s'));
                $where = array('dub_id' => $id);
                $result_update = $this->common_model->update($where);
    ...
    public function profile() {
        ...
        $this->form_validation->set_message('valid_email', '%s should be an valid email');
        $this->common_model->initialise('users');
        $where = array('id' => $this->session->userdata['moderator_user_id']);
        ...
        foreach ($data['result'] as $aRow) {
            if ($aRow['record_type'] == 1 && $aRow['isdub_moderate'] == 0) {
                ...
                $row[] = '<a href="' . base_url() . 'moderator/users/updatestatus/' . $aRow['dub_id'] . '/' . $aRow['dub_status'] . '"><button class="btn" title="status" style="border:1px solid #cccccc;">Status</button></a>&nbsp;&nbsp;'
                        . '<a href="' . base_url() . 'moderator/users/update/' . $aRow['dub_id'] . '"><button class="btn" title="update" style="border:1px solid #cccccc;">Update</button></a>&nbsp;&nbsp;';
	...
	public function getRData() { 
        ...
        foreach ($data['result'] as $aRow) {
            if ($aRow['record_type'] == 2 && $aRow['isdub_moderate'] == 0) {
                ...
                $row[] = '<a href="' . base_url() . 'moderator/users/updaterecstatus/' . $aRow['dub_id'] . '/' . $aRow['dub_status'] . '"><button class="btn" title="status" style="border:1px solid #cccccc;">Status</button></a>&nbsp;&nbsp;'
                        . '<a href="' . base_url() . 'moderator/users/recordupdate/' . $aRow['dub_id'] . '"><button class="btn" title="update" style="border:1px solid #cccccc;">Update</button></a>&nbsp;&nbsp;'
                        . '<a href="' . base_url() . 'moderator/users/moverecord/' . $aRow['dub_id'] . '"><button class="btn" title="move" style="border:1px solid #cccccc;">Move</button></a>';
                $output['aaData'][] = $row;
        }
    }
        if($this->input->get_post('sSearch')){
        ...
        foreach ($data['result'] as $aRow) {
            if ($aRow['record_type'] == 1 && $aRow['isdub_moderate'] == 0) {
                ...
                $i = $i + 1;
               	...
               	$row[4] = '<a href="' . base_url() . 'moderator/users/dubdelupdate/' . $aRow['dub_id'] . '"><button class="btn" title="update" style="border:1px solid #cccccc;">Update</button></a>&nbsp;&nbsp;';
        ...
        foreach ($data['result'] as $aRow) {
            if ($aRow['record_type'] == 2 && $aRow['isdub_moderate'] == 0) {
                ...
                $i = $i + 1;
               	...
               	$row[4] = '<a href="' . base_url() . 'moderator/users/dubdelrecupdate/' . $aRow['dub_id'] . '"><button class="btn" title="update" style="border:1px solid #cccccc;">Update</button></a>&nbsp;&nbsp;';
